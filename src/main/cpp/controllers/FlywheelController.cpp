...
void FlywheelController::SetGoal(double angularVelocity) {
    m_nextR << angularVelocity;
}

bool FlywheelController::AtGoal() { return m_atGoal; }

void FlywheelController::SetMeasuredAngularVelocity(double angularVelocity) {
    m_y << angularVelocity;
}

double FlywheelController::EstimatedAngularVelocity() const {
    return m_observer.Xhat(0);
}

double FlywheelController::AngularVelocityError() const {
    return m_r(0, 0) - m_observer.Xhat(0);
}

double FlywheelController::AngularVelocityGoal() const { return m_nextR(0, 0); }
...
void FlywheelController::Update(units::second_t dt,
    ...
    velocityLogger.Log(elapsedTime, m_y(0, 0), EstimatedAngularVelocity(),
                       AngularVelocityGoal());
    ...
    m_atGoal = std::abs(AngularVelocityError()) < kAngularVelocityTolerance;
    m_r = m_nextR;
